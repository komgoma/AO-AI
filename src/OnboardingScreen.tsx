...
const OnboardingScreen: React.FC<OnboardingProps> = ({ onStart, setAiName, aiName }) => {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-sky-700 via-cyan-200 to-yellow-200 relative overflow-hidden">
      <div className="absolute inset-0 bg-grid-white/[0.2] bg-[length:20px_20px] pointer-events-none"></div>
      
      {/* デザイン要素 */}
      <div className="absolute -top-24 -right-24 w-96 h-96 bg-gradient-to-br from-sky-400 to-cyan-500 rounded-full opacity-20 blur-3xl"></div>
      <div className="absolute -bottom-24 -left-24 w-96 h-96 bg-gradient-to-tr from-cyan-400 to-yellow-300 rounded-full opacity-20 blur-3xl"></div>
          ...
          <div className="p-8 md:p-12 text-center">
            ...
            <div className="max-w-md mx-auto">
              <div className="bg-white/30 backdrop-blur-sm p-6 rounded-2xl">
                ...
                <button
                  onClick={onStart}
                  className="w-full bg-gradient-to-r from-sky-500 to-cyan-500 text-white py-4 px-6 rounded-2xl font-medium flex items-center justify-center gap-2 hover:opacity-90 transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-[1.02]"
                >
                  AIクローンを作成する
                  <ArrowRight size={20} />
                </button>
